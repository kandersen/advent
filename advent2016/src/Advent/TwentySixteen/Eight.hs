{-# LANGUAGE RankNTypes #-}
module Advent.TwentySixteen.Eight(main) where
...
execute cs = runSTUArray start
  where
    ...
    interpret arr (RotateRow y n) = replicateM_ n $ do
...
main :: IO ()
main = defaultMain "2016.8" (pLines pCommand) $ \input -> do
  putStrLn "----[ Part  I ]----"
  let end = execute input
  print $ litPixels end
  putStrLn "----[ Part II ]----"
  display end
