...
findFirstPositionVisitedTwice sc =
  either id (^. _1) $ runIdentity $ runEitherT (execStateT (interpret sc) ((0,0), North, Set.empty))
  where
    interpret :: SantaCommand a -> StateT (Pos, Heading, Set Pos) (EitherT Pos Identity) a
    interpret (Pure a) = return a
    interpret (Free (Turn d a)) = do
      _2 %= applyTurn d
      interpret a
