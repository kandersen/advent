...
module Advent.Library.Trie where
...
insert (x:xs) Trie { .. } = 
  let mv = Map.lookup x children in
  case mv of
    Nothing -> Trie value (Map.insert x (insert xs (leaf False)) children)
...
insertCPS (x:xs) (Trie { .. }) k =
  let mv = Map.lookup x children in
  case mv of
    Nothing -> insertCPS xs (leaf False) $ \newChild ->
...
insertWithHandler h def (x:xs) Trie { .. } fk sk = 
  let mv = Map.lookup x children in
  case mv of 
    Nothing -> insertWithHandler h def xs (leaf def) fk $ \newChild ->
...
insert'' (x:xs) Trie { .. } fk sk = 
  let mv = Map.lookup x children in
  case mv of
    Nothing -> insert'' xs (leaf False) fk $ \newChild ->
                 sk (Trie value (Map.insert x newChild children))
...
insert''' (x:xs) n Trie { .. } fk sk = 
  let mv = Map.lookup x children in
  case mv of
    Nothing -> insert''' xs n (leaf Nothing) fk $ \newChild ->
                 sk (Trie value (Map.insert x newChild children))
